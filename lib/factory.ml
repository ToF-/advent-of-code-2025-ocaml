...
let switch_lights diagram lights = List.fold_left switch_light diagram lights
...
let rec perm l =
  ...
  | xs ->
      List.flatten
        (List.map (fun x -> List.map (List.cons x) (perm (del x xs))) xs)
