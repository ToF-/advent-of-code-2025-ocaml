...
let tests =
  ...
  >::: [
         ( "accessible rolls for sample (A)" >:: fun _ ->
           assert_equal ~printer:string_of_int 13 (accessible_rolls sample option_a) );
         ( "accessible rolls for input (A)" >:: fun _ ->
           assert_equal ~printer:string_of_int 1516 (accessible_rolls input option_a) );
         ( "accessible rolls for sample (B)" >:: fun _ ->
           assert_equal ~printer:string_of_int 43 (accessible_rolls sample option_b) );
         ( "accessible rolls for input (B)" >:: fun _ ->
           assert_equal ~printer:string_of_int 9122 (accessible_rolls input option_b) );
       ]
